    ...
    $.extend({
    	...
    	table: {
			...
			initTableHeight:function () {
				...
				var panelH = $(".container-div").height();
				//查询区域条件高度
				var serarchBarH = 0;
				if ($('.search-collapse').css('display') === 'none') {
					serarchBarH = 0;
				} else {
					serarchBarH = $(".search-collapse").height();
				}
				//拿到顶部工具栏高度
				var toolBarH = $(".bs-bars.pull-left").height();
				//计算表格container该设置的高度
				var height = panelH - toolBarH - serarchBarH - 13 - 5 - 10 - 20 - 15 - 70;
        ...
        operate: {
            ...
            successTabCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
    	            ...
    	            var $contentWindow = $('.SoftDing_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
    	            $.modal.close();
